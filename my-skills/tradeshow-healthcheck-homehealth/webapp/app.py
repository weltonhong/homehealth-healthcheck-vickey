...
Senior Care Marketing Max | Home Health Edition

Wraps the trade show health check (my-skills/tradeshow-healthcheck-homehealth)
into a browser UI. The rep enters agency + cities, clicks Run, and watches
...
st.set_page_config(
    page_title="Home Health Online Health Check | Senior Care Marketing Max",
    page_icon="🩺",
    layout="centered",
)
...
st.markdown("<h1 class='scmm-title'>Home Health Online Health Check</h1>", unsafe_allow_html=True)
st.markdown(
    "<div class='scmm-sub'>A 60-90 second snapshot of how the home health agency shows up online.</div>",
    unsafe_allow_html=True,
)
...
with st.form("health_check_form", clear_on_submit=False):
    business = st.text_input("Home Health Agency Name", placeholder="e.g., Bayada Home Health Care")
    home_city = st.text_input("Home City", placeholder="e.g., Philadelphia")
    state = st.selectbox("State", US_STATES, index=US_STATES.index("PA"))
...
def _validate(values):
    missing = []
    for label, key in [
        ("Home Health Agency Name", "business"),
        ("Home City", "home_city"),
        ("State", "state"),
    ]:
...
if st.session_state.get("scan_started") and not st.session_state.get("scan_done"):
    ...
    if missing:
        ...
        log_lines = []
        spinner_box.info("Scanning... reviews, 3-Pack (home health + home health agency), SEO, ads, website running in parallel.")
        ...
        with st.expander("Website", expanded=False):
            ...
            if web.get("unverified"):
                ...
                for k, label in [
                    ...
                    ("medicare_certified", "Medicare-certified mention"),
                    ("services_list", "Services list (skilled nursing, PT, OT, ST...)"),
                    ("referral_page", "Referral source page"),
                    ("cms_star_rating", "CMS Star Rating displayed"),
                ]:
                    v = checks.get(k)
                    mark = "✅" if v is True else ("❌" if v is False else "—")
                    st.write(f"- {label}: {mark}")
                services = web.get("services_offered") or []
                if services:
                    st.write(f"&nbsp;&nbsp;↳ Services detected: {', '.join(services[:6])}")
                star_value = web.get("cms_star_rating")
                if star_value:
                    st.write(f"&nbsp;&nbsp;↳ CMS Star Rating value: {star_value}")
